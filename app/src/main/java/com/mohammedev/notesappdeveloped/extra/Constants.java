package com.mohammedev.notesappdeveloped.extra;

import android.graphics.Color;

public class Constants
{
    public static final int NORMAL_NOTE = 0;
    public static final int PHOTO_NOTE = 1;
    public static final int CHECK_NOTE = 2;

    public static final String EXTRA_PHOTO_URI = null;
    public static final String THE_NOTE = "a";
    public static final String CHECK_BOX_TEXT = null;
    public static final String COLOR = "";
    public static final int GREEN_COLOR = Color.GREEN;

    public static final String EXTRA_ID = "com.mohammedev.notesappdeveloped.extraid";
    public static final String EXTRA_NOTE_TEXT = "com.mohammedev.notesappdeveloped.text";
}
